...
void track::ui::CustomLookAndFeel::drawLinearSlider(
    ...
    Slider &slider) {
        ...
        auto trackWidth =
            jmin(6.0f, slider.isHorizontal() ? (float)height * 0.25f
                                             : (float)width * 0.25f);
        ...
        g.setColour(Colour(0xFF474747)); // fill color
        ...
        if (!isTwoVal) {
           ...
           // draw thumb outline 
            ...
            g.drawEllipse(Rectangle<float>(static_cast<float>(thumbWidth),
                                           static_cast<float>(thumbWidth))
                              .withCentre(isThreeVal ? thumbPoint : maxPoint), 1.f);
        }
        ...
        if (isTwoVal || isThreeVal) {
            ...
            if (slider.isHorizontal()) {
                ...
                drawPointer(g,
                            jmax(0.0f, (float)x + (float)width * 0.5f -
                                           trackWidth * 2.0f),
